...
def init_supabase():
 ...
 st.session_state.supabase.table("financial_data").select("*", 
...
def save_financial_data(symbole, annee, data_dict):
 ...
 return False
 try:
 record = {'symbole': symbole, 'annee': annee, 'data': data_dict, 'last_update': 
datetime.now().isoformat()}
 existing = supabase.table("financial_data").select("*").eq("symbole", 
symbole).eq("annee", annee).execute()
 if existing.data:
 supabase.table("financial_data").update(record).eq("symbole", 
symbole).eq("annee", annee).execute()
 else:
...
def delete_financial_data(symbole, annee):
 ...
 return False
 try:
 supabase.table("financial_data").delete().eq("symbole", symbole).eq("annee", 
annee).execute()
 return True
 except Exception as e:
...
def validate_financial_data(bilan, compte_resultat):
 ...
 errors.append(f" Bilan non équilibré : {bilan.get('actif_total', 0) -
bilan.get('passif_total', 0):,.0f} FCFA")
...
def calculate_enhanced_financial_ratios(bilan, compte_resultat, flux_tresorerie):
 ...
 fcf = flux_tresorerie.get('flux_exploitation', 0) + flux_tresorerie.get('flux_investissement', 
0)
 ...
 # RENTABILITÉ
 marge_nette = safe_ratio(compte_resultat.get('resultat_net', 0), 
compte_resultat.get('chiffre_affaires', 0))
 ...
 # LIQUIDITÉ
 ratio_liquidite_generale = safe_ratio(bilan.get('actif_courant', 0), 
bilan.get('passif_courant', 0))
 ...
 ratio_liquidite_immediate = safe_ratio(bilan.get('tresorerie', 0), 
bilan.get('passif_courant', 0))
 if ratio_liquidite_immediate is not None:
 ratios['ratio_liquidite_immediate'] = ratio_liquidite_immediate
 
 # ENDETTEMENT
 ratio_endettement = safe_ratio(bilan.get('dettes_totales', 0), 
bilan.get('capitaux_propres', 0))
...
0)))
 ...
 # EFFICACITÉ
 rotation_actifs = safe_ratio(compte_resultat.get('chiffre_affaires', 0), 
bilan.get('actif_total', 0))
 if rotation_actifs is not None:
 ratios['rotation_actifs'] = rotation_actifs
 
 rotation_stocks = safe_ratio(compte_resultat.get('chiffre_affaires', 0), 
bilan.get('stocks', 0))
 if rotation_stocks is not None:
 ratios['rotation_stocks'] = rotation_stocks
 
 delai_recouvrement = safe_ratio(bilan.get('creances', 0), 
compte_resultat.get('chiffre_affaires', 0))
 ...
 # FLUX DE TRÉSORERIE
 qualite_benefices = safe_ratio(flux_tresorerie.get('flux_exploitation', 0), 
compte_resultat.get('resultat_net', 0))
...
def calculate_valuation_multiples(symbole, annee, ratios_entreprise, financial_data):
 ...
 if cours_actuel > 0:
 valorisations['ecart_per'] = ((juste_valeur_per - cours_actuel) / cours_actuel) * 
100
 ...
 cours_actuel = ratios_entreprise.get('cours_action', 0)
 if cours_actuel > 0:
 valorisations['ecart_ev_ebitda'] = ((juste_valeur_ev_par_action - cours_actuel) / 
cours_actuel) * 100
...
def calculate_financial_projections(symbole, financial_data, annees_projection=3):
 ...
 tcam_rn = calcul_tcam(abs(rn_values[0]), abs(rn_values[-1]), len(rn_values) - 1) if 
rn_values[0] != 0 else 0
 ...
 'methode': f'Pondération dynamique (TCAM {poids_tcam*100:.0f}% + Régression 
{poids_reg_ca*100:.0f}%)'
 }
# ===========================
# SCRAPING BRVM
...
def developer_section():
 ...
 with col1:
 symbole = st.text_input("Symbole de l'action (ex: SNTS, SGBC, BICC)", 
key="symbole_input").upper()
 with col2:
 ...
 st.warning(f" {nom_societe} - Cours: {cours_brvm:,.0f} FCFA ({variation}%)")
 else:
 st.info(f" {nom_societe} - Cours: {cours_brvm:,.0f} FCFA")
 elif symbole_existe:
 st.info(f" Symbole {symbole} trouvé - Cours: {cours_brvm:,.0f} FCFA")
 else:
 st.warning(f" Symbole {symbole} non trouvé dans les données BRVM")
 
 tab1, tab2, tab3, tab4 = st.tabs([" Bilan", " Compte de Résultat", " Flux de 
Trésorerie", " Ratios Calculés"])
...
0)), 
 ...
 value=float(existing_data.get('bilan', {}).get('capitaux_propres', 
0)), 
 step=1000000.0,
 format="%.0f",
...
0)), 
 ...
 st.error(f" Bilan non équilibré ! Différence: {actif_total - passif_total:,.0f} 
FCFA")
 else:
...
{}).get('charges_exploitation', 0)), 
 ...
 dotations_amortissements = st.number_input("Dotations aux Amortissements 
(FCFA)", 
 value=float(existing_data.get('compte_resultat', 
...
{}).get('flux_financement', 0)), 
 ...
 ratios = calculate_enhanced_financial_ratios(bilan_data, compte_resultat_data, 
flux_tresorerie_data)
 ...
 st.warning(f" **{interp}** - Surveillance nécessaire")
 else:
 ...
 'Dernière MAJ': data.get('last_update', 'N/A')[:19] if data.get('last_update') 
else 'N/A'
 })
...
index=len(annees)-1)
 ...
 # RATIOS
 st.markdown(f"### Ratios pour {symbole_selected} -
{annee_selectionnee}")
 ...
 score, details = calculate_piotroski_score(symbole_selected, 
annee_selectionnee, financial_data)
 ...
 projections = calculate_financial_projections(symbole_selected, 
financial_data)
 ...
 st.caption(f"TCAM CA: {projections.get('tcam_ca', 0):.2f}% | Fiabilité R²: 
{projections.get('r2_ca', 0):.2%}")
 else:
 st.warning(projections.get('erreur', 'Erreur de projection'))
 
 else:
 st.warning(f" Aucune donnée pour {symbole_selected}")
...
def main():
 ...
 "Navigation",
 [" Accueil & Cours", " Section Développeur", " Guide d'Utilisation", " À 
propos"]
 )
 ...
 ** Nouveautés de cette version :**
 ...
 st.markdown("---")
 st.caption(f"Source : BRVM | Cloud : Supabase | 
 ...
 **Important** : Les amortissements sont requis pour un calcul correct de 
l'EBITDA !
 ...
 **Application d'analyse fondamentale avancée pour la Bourse Régionale des 
Valeurs Mobilières**
 ...
 **Valorisation améliorée :**
 ...
 Pour toute question ou suggestion d'amélioration, utilisez le bouton de feedback 
Streamlit.
 ...
 **Licence** : Usage personnel et éducatif
 ...
 st.info(" **Astuce** : Utilisez le mode sombre pour une meilleure expérience ( 
→ Settings → Theme)")
